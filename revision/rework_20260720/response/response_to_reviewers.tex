...
The manuscript is provided as a clean revised version, as requested. Additional file 1 now contains 24 supplementary tables and six supplementary figure groups; Additional file 2 contains 54 data sheets plus a README, sheet index, and data dictionary. The two added audit sheets document the archived condition-level table and the corrected g:Profiler identifier alignment. Run-level and large machine files remain available in the repository rather than being reduced to undocumented summaries.
...
\RevisedText \qtext{The evidence does not support claims that the dual-stream model is superior to a fingerprint MLP, that the chemical-only models predict condition-specific responses, that reversal-associated genes are direct drug targets, that RG2833 or Tianeptinaline/BG-1010 has stable measured support, that GDSC provides validation, or that docking proves HDAC engagement.}

\Location Revised manuscript pp.~1--2, lines 14--55 (Abstract); pp.~4--12, lines 104--328 (Methods and evidence hierarchy); p.~25 and pp.~27--30, lines 507--619 (Discussion, limitations, and Conclusions; Figure~9 occupies p.~26). Additional files 1 and 2 document the evidence chain.
...
\Location Revised manuscript pp.~5--6, lines 132--154 and Table~1; pp.~12--13, lines 330--347. Additional file 1, Tables S1--S3; Additional file 2, sheets \texttt{01\_Dataset}, \texttt{02\_Split\_Audit}, \texttt{07\_Candidate\_Identity}, \texttt{10\_Official\_Conditions}, and \texttt{21\_Condition\_Manifest}.
...
\Location Revised manuscript pp.~6--8, lines 155--219 (architecture and chemical-only scope); pp.~14--16, lines 362--395 and Table~2 (repeated benchmarks and strict LDO); p.~25 and p.~27, lines 507--535 (interpretation; Figure~9 occupies p.~26). Additional file 1, Tables S4--S6 and S11; Additional file 2, sheets \texttt{03\_Model\_Runs}--\texttt{05\_Model\_Paired} and \texttt{15\_LDO\_Membership}--\texttt{18\_LDO\_Paired}.
...
\Location Revised manuscript pp.~10--11, lines 276--296 (gene/network methods); pp.~21--23, lines 454--475 and Figures~6--7 (results). Additional file 1, Tables S17--S20 and Figure S4; Additional file 2, sheets \texttt{36\_Reversal\_Genes}--\texttt{43\_Pathway\_Overlap}, including \texttt{41A\_gProfiler\_Audit}.
...
\Action Under signed wTRS, the explicit class I HDAC subset was enriched at the fixed top 0.5\%, 1\%, 5\%, and 10\% revision cutoffs (fold 30.6, 19.2, 8.46, and 4.62; all within-family FDR $<10^{-4}$). For each metric, the ranking equally averages 528 library-percentile observations per compound (24 screening runs $\times$ 22 cancers). The co-primary Spearman reversal metric was not significantly enriched. Stability is primary across 48 configurations; the legacy-inclusive 72 configurations are sensitivity only. Candidate roles are tiered rather than presented as universally robust.
...
\Location Revised manuscript pp.~1--2, lines 33--55; pp.~13--14, lines 348--361 and Figure~1; pp.~18--21, lines 426--453 and Table~3; p.~25 and pp.~27--28, lines 507--565 (Figure~9 occupies p.~26). Additional file 1, Tables S14 and S16; Additional file 2, sheets \texttt{06\_HDAC\_Enrichment} and \texttt{29\_Stability\_Primary48}--\texttt{32\_Stability\_BySplit}.
...
\Location Revised manuscript pp.~6--10, lines 155--267 (models, splits, and official-condition methods); pp.~12--18, lines 329--425 and Figures~1--3 (results); pp.~28--30, lines 566--619 (limitations and Conclusions). Additional file 1, Tables S3--S13; Additional file 2, generalization, holdout, LDO, official-condition, and predicted--measured sheets.
...
\Location Revised manuscript pp.~1--4, lines 14--121; all figure captions; p.~25 and pp.~27--30, lines 507--619 (Figure~9 occupies p.~26). Additional file 1, evidence-boundary preamble and Tables S17--S24.
...
\Location Revised manuscript pp.~3--4, lines 75--121; pp.~6--7, lines 155--191; p.~25 and p.~27, lines 507--535 (Figure~9 occupies p.~26).
...
\Response We agree. The original single-run values are not retained as evidence of superiority. The old 0.2841 value could not be linked to a reproducible run artifact and appeared only as a literal value in a legacy ablation script; it is therefore excluded from the revision.
...
\Location Revised manuscript pp.~6--10, lines 155--267; pp.~14--18, lines 362--425 and Table~2/Figure~3; p.~25 and pp.~27--29, lines 507--595 (Figure~9 occupies p.~26). Additional file 1, Tables S3--S13.
...
\Location Revised manuscript pp.~9--10, lines 248--267 (official-condition methods); pp.~16--18, lines 396--425 and Figures~2--3 (measured results); pp.~28--29, lines 566--600 (limitations). Additional file 1, Tables S8, S12, and S13; Additional file 2, measured and predicted--measured sheets, including \texttt{21\_Condition\_Manifest}.
...
\Action Mocetinostat is core because it combines strict leave-drug testing, no corrected-HDAC training scaffold, maximum training similarity 0.537, eight HiQ profiles, positive measured reversal, and a primary 48-configuration median stability percentile of 92.2. NCH-51 is secondary (eight HiQ profiles; percentile 88.5; residual scaffold exposure). TC-H-106 is exploratory (four HiQ profiles; percentile 84.3; close neighbor). Tianeptinaline/BG-1010 remains excluded regardless of its descriptive rank.

\RevisedText \qtext{TC-H-106 had four high-quality profiles, a primary stability percentile of 84.3, and a closest corrected annotation-defined HDAC training neighbor of 0.789. It was downgraded to exploratory/original-method-sensitive rather than selected as the principal candidate.}

\Location Revised manuscript pp.~18--21, lines 426--453, Table~3, and Figures~4--5; pp.~21--24, lines 454--486 (candidate biology and DepMap context); pp.~27--28, lines 536--565. Additional file 1, Tables S7--S16.
...
\Location Revised manuscript pp.~11--12, lines 305--320 (protocol); pp.~24--26, lines 487--506 and Figure~9 (results); pp.~28--29, lines 566--600 (limitations). Additional file 1, Table S22 and Figure S6; Additional file 2, sheets \texttt{48\_Redocking}--\texttt{51\_Receptor\_Sensitivity}.
...
\Location Revised manuscript p.~4, lines 104--121 (evidence exclusions); pp.~28--29, lines 566--600 (limitations); pp.~29--30, lines 601--619 (Conclusion). Additional file 1, Table S23; Additional file 2, sheet \texttt{52\_External\_Sensitivity}.
...
\Location Revised manuscript pp.~3--4, lines 89--103; pp.~23--24, lines 476--486; pp.~27--30, lines 536--619.
...
\Location Revised manuscript pp.~6--12, lines 155--328; pp.~14--16, lines 362--381. Additional file 1, Tables S2--S6 and S24; Additional file 2, sheets \texttt{03\_Model\_Runs}, \texttt{04\_Model\_Summary}, and \texttt{53\_Reproducibility}.
