...
\input{generated/table_S03}

\input{generated/table_S04}

\input{generated/table_S05}

\input{generated/table_S06}
...
\input{generated/table_S07}

\input{generated/table_S08}
\end{landscape}
...
\input{generated/table_S10}

\input{generated/table_S11}
...
\input{generated/table_S14}

\input{generated/table_S15}
\end{landscape}
...
\input{generated/table_S16}

Formal testing shows that the explicitly annotated class I subset is enriched under signed wTRS at every fixed revision cutoff, but not under Spearman reversal. For each metric, compounds were ordered by the mean of 528 equally weighted library-rank percentiles (24 screening runs across four generalization regimes, two models, and three seeds, multiplied by 22 cancers). The cutoffs were fixed for the revision analysis but were not prospectively preregistered. The Results and conclusions therefore describe the enrichment as metric-dependent rather than universal. The retained original title refers to computational prioritization and does not imply model superiority, efficacy, or metric-independent enrichment. The broader annotation-defined HDAC analysis and the legacy metric are included in Additional file 2.
...
\input{generated/table_S17}

\input{generated/table_S18}
\end{landscape}

\input{generated/table_S19}
...
Additional file 2 contains the complete run-level, candidate-level, condition-level, network, pathway, DepMap, docking, sensitivity, and reproducibility tables with filters and a data dictionary. A dedicated condition-level manifest records the 83,490-row archived condition table, its schema, and why that large table is represented by auditable aggregates rather than duplicated into the formatted workbook. Repository-relative source paths are used; no AutoDL absolute paths are required to interpret the workbook. Blank fields are retained only when a measurement is not applicable or was not recorded, and the corresponding reason is supplied in a status or note column.
