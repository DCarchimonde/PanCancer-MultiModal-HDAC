...
\begin{minorrevision}
\noindent\textbf{Keywords:} molecular representation; transcriptomic reversal; drug repurposing; LINCS L1000; TCGA; histone deacetylase; leakage-aware evaluation; measured perturbation.\par
\end{minorrevision}
\vspace{1em}
...
\begin{minorrevision}
Training time reflected the larger dual-stream architecture. Depending on split, mean end-to-end training and evaluation time was 152--303 seconds per dual-stream run and 46--76 seconds per fingerprint run on the RTX 4090. Early stopping selected mean epochs between 3.0 and 10.0 for the dual stream and 3.7 and 11.7 for the fingerprint model across settings. These observed costs and every run-level value are reported in Additional file 1.\par
\end{minorrevision}
...
Across 176 candidate--cancer pairs, corrected annotation-defined HDAC ensemble predictions were positively associated with official high-quality measured reversal (Figure~\ref{fig:predmeasured}). For signed wTRS, the dual-stream model had Pearson $r=0.642$ (crossed-bootstrap 95\% CI 0.365--0.838) and Spearman $\rho=0.640$ (0.297--0.840), whereas the fingerprint MLP had $r=0.805$ (0.601--0.911) and $\rho=0.803$ (0.554--0.919). For rank-based reversal, the dual-stream values were $r=0.813$ (0.634--0.919) and $\rho=0.801$ (0.594--0.909); fingerprint values were $r=0.853$ (0.710--0.921) and $\rho=0.830$ (0.653--0.910). The crossed bootstrap resampled both eight candidates and 22 cancers, so it does not treat the 176 pairs as independent. Every interval remained above zero, and leave-one-candidate-out and leave-one-cancer-out diagnostics retained positive coefficients. These results support the prediction-to-measurement bridge for both encoders while providing no evidence that the dual-stream architecture is superior. They also remain internal to held-out compounds from the LINCS resource rather than constituting validation on an independent experimental platform.
