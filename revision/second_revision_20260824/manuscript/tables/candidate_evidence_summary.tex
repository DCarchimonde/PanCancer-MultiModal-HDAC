\begin{table}[H]
\centering
\caption{\textbf{Frozen candidate evidence tiers.} \roundtwotext{Corrected-HDAC profile $n$ is the number of quality-controlled modeling signatures assigned to the candidate in the corrected annotation-defined HDAC test set; HiQ $n$ is the number of official LINCS high-quality profiles.} Stability is the median strength percentile (100 = strongest) across the 48 primary split--model--seed--metric configurations using signed wTRS and Spearman reversal; the 72-configuration legacy-inclusive sensitivity analysis is reported in Additional file 2. Identity and exposure rules override descriptive rank.}
\label{tab:candidateevidence}
\resizebox{\textwidth}{!}{%
\begin{tabular}{llrrrrl}
\toprule
Candidate & Frozen role & \roundtwotext{Corrected-HDAC profile $n$} & Max train similarity & HiQ $n$ & Stability percentile & Evidence interpretation \\
\midrule
