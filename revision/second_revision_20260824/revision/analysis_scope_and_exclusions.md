# Revised analysis scope and exclusions
...
- Signed wTRS is amplitude-sensitive; the enrichment may partly reflect perturbational magnitude and is not generalized across reversal definitions.
...
The eight-compound predicted--measured analysis establishes transcriptomic concordance only. Without a condition-matched non-HDAC comparator set, it does not establish preferential validation relative to alternative perturbagens. The exact artifact and rerun boundary is documented in [`../REPRODUCIBILITY.md`](../REPRODUCIBILITY.md).
